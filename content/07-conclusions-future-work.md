...
each of the five objectives stated in §1.1 (statement of purpose);
§7.2 draws broader conclusions that do not map onto a single
objective; §7.3 records the limitations honestly and in the same
place as the conclusions they qualify; §7.4 lists future-work
directions; §7.5 documents a hand-over protocol for the next
AUCOOP cohort.
...
The five objectives of §1.1 (statement of purpose) were formulated
jointly with the companion thesis. Each is checked here against the
evidence assembled in Chapters 3 (methodology) to 6 (sustainability).
...
AUCOOP Community-Network-Handbook (see §3.C, the AUCOOP Handbook,
and Appendix A, the online Handbook) is a public Git repository
whose Markdown sources build to both a public website and a
downloadable PDF, and whose contribution rules
(`.opencode/rules/`, `AGENTS.md`) are explicit enough that a
volunteer who has never seen the project can open a pull request
the same day. The validation lens here is whether the artefact
*continued to grow during the deployment*, and §4.4 (teacher
seminar) records the qualitative answer: commits and pull requests
merged during the field phase, with the Namibia case study itself
authored against the handbook's own contribution conventions.
...
use case) / Ch3 (recipe guide) split (see §3.C.4, dual output) is
the structural expression of this objective: Ch2 reads as a story
for the non-specialist, Ch3 reads as a how-to for the volunteer
doing the work. The reservations are concrete and named in §4.6
(lessons consolidated back into the handbook): the *Antennas* and
*Power-and-UPS* recipes remain stubs, and the laptop chapter would
benefit from a richer photographic walkthrough of the Basic
Input/Output System (BIOS) configuration step. None of these
reservations invalidate the structural claim; they identify the
next contribution window.
...
reference for new cohorts (the hand-over protocol of §7.5 (next
cohort) operationalises this), and the deployment validated in
Chapter 4 (results) was executed by a team that used the handbook
as its primary reference during the field phase. External
validation — adoption by a sister organisation, a fork by a
different community-network group — is the honest gap and is listed
in §7.4 (future work).
...
in Namibia. *Met.* Chapter 4 (results) reports the validation in
detail using three lenses (coverage, sufficiency, adaptation). The
headline is that the handbook covered the deployment, was sufficient
for the operational decisions taken in the field, and absorbed the
adaptations required without structural rewrites. The validation is
intentionally qualitative (see §4.7.2, limits) because the recipe
is intended to support volunteer decision-making rather than to
deliver a quantitative performance contract; a quantitative
complement is listed in §7.4 (future work).
...
continuity problem (see §2.5, living documentation as an
engineering practice); the contribution of this work is to treat it
with the same tooling discipline as any other engineering problem:
version control, contribution rules, automated build, Continuous
Integration (CI), explicit governance. The Namibia deployment is
the validation instance, but the durable contribution is the
...
explicitly (see §3.A.4, hardware selection, and §3.A.6, wireless
mesh design), and the budget chapter quantifies what it is worth
(see §5.9, comparison: ~3.3× cash multiple over the commercial
...
dimensions (see §6.5, indirect effects of the open handbook): every
component not deployed is manufacturing carbon not incurred, cash
not raised, and operational burden not placed on the partner
organisation.
...
manufacturing-phase emissions (see §6.2, environmental dimension),
the open handbook engineers knowledge continuity into a volunteer
association (see §6.3, social and governance dimension), and the
donation-leveraged budget makes the recipe replicable on a small
NGO grant (see §6.4, economic dimension). The three-dimension
reading proposed by [Roura et al., 2026] for refurbished-ICT
digital-inclusion programmes maps cleanly onto the AUCOOP case and
gives the work an analytical vocabulary that an environment-only
chapter could not have provided.
...
boundary itself is documented in §1.4 (methods and procedures) of
both theses and in the shared bibliography backbone of Chapter 8
(bibliography).
...
is listed in §7.4 (future work). Until that test is run, the
validation should be read as
*existence-of-recipe-that-works-once*, not as
*generalisation-across-contexts*.

**Qualitative validation rather than quantitative.** Chapter 4
(results) deliberately presents the validation through the
qualitative lenses of coverage, sufficiency and adaptation (see
§4.7.2, limits). Quantitative characterisation of the deployed
network — per-link iperf, per-AP association counts, latency under
load, longitudinal uptime — was partially measured in the field but
is not consolidated in this document at submission time. The
quantitative complement is listed in §7.4 (future work).

**Engineering effort excluded from the cash budget.** The €6.5 k
cash figure of §5.7 (total) is the figure an organisation actually
has to raise to reproduce the recipe; the €49 k full-project figure
is what the same recipe would cost at indicative engineering rates.
...
**Refurbishment supply is a dependency on a partner network.** The
endpoint pipeline of §3.B (endpoint reconditioning and mass
deployment) presupposes a working donation pipeline (Labdoo,
NexTReT, the Foundawtion container). Sites without access to that
pipeline would face either a higher cost (commercial refurbishment
market) or a longer collection lead time. The recipe documents what
to do with the donations; it does not document how to bootstrap the
donation pipeline itself. That work is upstream of this thesis and
is owned by AUCOOP at the organisational level.

**Sustainability analysis is below the device-level scenario LCA
standard.** Chapter 6 (sustainability) uses per-unit figures from
public sources, applies them to the deployment, and reports the
result as an order-of-magnitude check across environmental, social
and economic dimensions. A proper study along the device-level
scenario LCA lines set by [Roura et al., 2026] would net transport,
...
The omission is documented in §6.7 (limits of this analysis).

**External adoption of the handbook is not yet measured.** The
handbook validation of §4.4 (teacher seminar) is internal to AUCOOP.
Adoption by a sister organisation — a fork, a citation, a
derivative recipe — would be the strongest single signal that the
centralisation objective (Objective 5) generalises, and is the
validation that the present thesis cannot deliver because the
handbook is too new.
...
remain stubs (consolidated in §4.6, lessons consolidated back into
the handbook). Both have field experience that has not yet been
written up; the next cohort can convert that experience into
recipes without starting from a blank page.
...
sequence would close it (see §4.6, lessons).
...
the LCA-grade limitation of §6.7 (limits of this analysis). The
natural shape is a device-level scenario LCA along [Roura et al.,
2026] lines — Allocation at the Point of Substitution (APOS),
Power-On-Hours-based attribution across successive users,
per-hour-of-service functional unit — extended with the social
(measured ICT access hours) and economic (technician / volunteer
hours, donor-pipeline cash multiplier) indicators that the same
study proposes. The N Mutschuana Primary School (NMPS) deployment
is precisely the low-income, community-scale empirical case that
paper identifies as under-represented in the existing data, and
would benefit from being written up as a contribution to that
literature. Likely a separate thesis or a collaboration with the
eReuse project rather than a chapter extension.
...
1. **Read** Chapter 1 (introduction: objectives, methodology),
   §3.C (the AUCOOP Handbook architecture), and Chapter 4 (results:
   validation lessons) of the present thesis, and the equivalent
   chapters of [Motje, 2026].
...
5. **Follow** the pull-request template. Tag a current maintainer
   ...
   tooling described in §3.C.5 (contribution model and AI-assisted
   authoring) is available to help with first drafts.
6. **Commit** any new operational lesson back into the handbook in
   ...
   surveyed in §1.2 (motivation) was waiting until "later" to write
   things down. Do not wait until later.
